...
// bean.setModifiedDateTime(rs.getTimestamp(10));
	 ...
	 * new StringBuffer( "SELECT * FROM ST_COLLEGE WHERE 1=1");
	 * 
	 * if (bean != null) { if (bean.getId() > 0) { sql.append(" AND id = " +
	 * bean.getId()); } if (bean.getName() != null && bean.getName().length() > 0) {
	 ...
	 * }
	 * 
	 * // if page size is greater than zero then apply pagination if (pageSize > 0)
	 * { // Calculate start record index pageNo = (pageNo - 1) * pageSize;
	 * 
	 * sql.append(" Limit " + pageNo + ", " + pageSize); // sql.append(" limit " +
	 * pageNo + "," + pageSize); }
	 * 
	 * ArrayList list = new ArrayList(); Connection conn = null; try { conn =
	 ...
	 * 
	 * public List search(CollegeBean bean) throws ApplicationException { return
	 * search(bean, 0, 0); }
	 * 
	 * public List list() throws ApplicationException { return list(0, 0); }
	 * 
	 * public List list(int pageNo, int pageSize) throws ApplicationException {
