...
public class MarksheetModel {
	...
	public void update(MarksheetBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement stmt = conn.prepareStatement(
					"update st_marksheet set roll_no=?,student_id=?,name=?,physics=?,"
					+ "chemistry=?,math=?,created_by=?,modified_by=?,created_datetime=?,modified_datetime=? where id=?");
	...
	public List getMeritList(int pageNo, int pageSize) throws ApplicationException {
		...
		StringBuffer sql = new StringBuffer(
				"select id,roll_no,name,physics,chemistry,math,(physics+chemistry+math)as total from st_marksheet where physics>33 and chemistry>33 and math>33 order by total desc");
		if (pageSize > 0) {
			pageNo = (pageNo - 1) * pageSize;
			sql.append(" limit " + pageNo + "," + pageSize);
		}
